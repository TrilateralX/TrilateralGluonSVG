...
Float EntryPoint_obj::processEvents(){
	while(true){
		::haxe::EntryPoint_obj::mutex->acquire();
		 ::Dynamic f = ::haxe::EntryPoint_obj::pending->shift();
		::haxe::EntryPoint_obj::mutex->release();
		if (::hx::IsNull( f )) {
			goto _hx_goto_0;
		}
		f();
	...
	_hx_goto_0:;
	Float time = ::haxe::MainLoop_obj::tick();
	bool _hx_tmp;
	if (!(::haxe::MainLoop_obj::hasEvents())) {
		_hx_tmp = (::haxe::EntryPoint_obj::threadCount == 0);
	}
	else {
		_hx_tmp = false;
	}
...
void EntryPoint_obj::run(){
	while(true){
		Float nextTick = ::haxe::EntryPoint_obj::processEvents();
		if ((nextTick < 0)) {
			goto _hx_goto_1;
		}
		if ((nextTick > 0)) {
			::haxe::EntryPoint_obj::sleepLock->wait(nextTick);
		}
	}
	_hx_goto_1:;
}
...
{
{
	...
	sleepLock =  ::sys::thread::Lock_obj::__alloc( HX_CTX );
}
{
	HX_JUST_GC_STACKFRAME
	mutex =  ::sys::thread::Mutex_obj::__alloc( HX_CTX );
}
{
	pending = ::Array_obj< ::Dynamic>::__new();
}
{
	threadCount = 0;
}
