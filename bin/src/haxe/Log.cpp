...
void _hx_run( ::Dynamic v, ::Dynamic infos){
	bool _hx_tmp;
	if (::hx::IsNotNull( infos )) {
		_hx_tmp = ::hx::IsNotNull( infos->__Field(HX_("customParams",d7,51,18,ed),::hx::paccDynamic) );
	}
	else {
		_hx_tmp = false;
	}
	if (_hx_tmp) {
		::String extra = HX_("",00,00,00,00);
		{
			int _g = 0;
			::cpp::VirtualArray _g1 = ( (::cpp::VirtualArray)(infos->__Field(HX_("customParams",d7,51,18,ed),::hx::paccDynamic)) );
			while((_g < _g1->get_length())){
				 ::Dynamic v = _g1->__get(_g);
				_g = (_g + 1);
				extra = (extra + (HX_(",",2c,00,00,00) + ::Std_obj::string(v)));
			}
		}
		__trace((::Std_obj::string(v) + extra),infos);
	}
	else {
		__trace(( (::String)(v) ),infos);
	}
}
